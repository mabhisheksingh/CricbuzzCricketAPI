...
public class CustomLogger {
    ...
    public static void logOutRequest(ServerRequest serverRequest, ServerResponse sr) {
        logger.info("Id: {}, response status code {}, Http headers {}, path variables {}",
                serverRequest.attributes().get("org.springframework.web.server.ServerWebExchange.LOG_ID"),
                sr.statusCode(), sr.headers(), sr.toString());
    }

    public static void logOutRequest(ServerRequest serverRequest, Throwable sr) {
        logger.info("Id: {}, response status code {}, path variables {}",
                serverRequest.attributes().get("org.springframework.web.server.ServerWebExchange.LOG_ID"),
                sr.getMessage(), sr.toString());
